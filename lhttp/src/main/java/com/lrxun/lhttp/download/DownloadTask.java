...
import com.lrxun.lhttp.LDownload;
import com.lrxun.lhttp.db.DownloadManager;
import com.lrxun.lhttp.exception.HttpException;
import com.lrxun.lhttp.exception.OkGoException;
import com.lrxun.lhttp.exception.StorageException;
...
public class DownloadTask implements Runnable {
    ...
    public DownloadTask(String tag, Request<File, ? extends Request> request) {
        ...
        progress.tag = tag;
        progress.folder = LDownload.getInstance().getFolder();
        progress.url = request.getBaseUrl();
        progress.status = Progress.NONE;
        progress.totalSize = -1;
        progress.request = request;

        executor = LDownload.getInstance().getThreadPool().getExecutor();
    ...
    public DownloadTask folder(String folder) {
        if (folder != null && !TextUtils.isEmpty(folder.trim())) {
            ...
            BulletLogger.w("folder is null, ignored!");
        }
        return this;
    }
    ...
    public DownloadTask fileName(String fileName) {
        if (fileName != null && !TextUtils.isEmpty(fileName.trim())) {
            ...
            BulletLogger.w("fileName is null, ignored!");
        }
        return this;
    }
    ...
    public void start() {
        if (LDownload.getInstance().getTask(progress.tag) == null || DownloadManager.getInstance().get(progress.tag) == null) {
        ...
        if (progress.status == Progress.NONE || progress.status == Progress.PAUSE || progress.status == Progress.ERROR) {
            ...
            BulletLogger.w("the task with tag " + progress.tag + " is already in the download queue, current task status is " + progress.status);
        }
    }
    ...
    public DownloadTask remove(boolean isDeleteFile) {
        ...
        DownloadTask task = LDownload.getInstance().removeTask(progress.tag);
